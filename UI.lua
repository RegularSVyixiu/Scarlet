...
ui.MakeDraggable = function(obj, dragObj, smoothness)
	...
	Storage.InputMoved[dragObj] = localMouse.Move:Connect(function()
		if dragging then
			TweenService:Create(obj, TweenInfo.new(math.clamp(smoothness, 0, 1), Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), { Position = UDim2.new(0, localMouse.X - startPos.X, 0, localMouse.Y - startPos.Y) }):Play()
		end
	end)
end

ui.UnMakeDraggable = function(dragObj)
	if Storage.InputBegan[dragObj] ~= nil and Storage.InputEnded[dragObj] ~= nil and Storage.InputMoved[dragObj] ~= nil then
		Storage.InputBegan[dragObj]:Disconnect()
		Storage.InputEnded[dragObj]:Disconnect()
		Storage.InputMoved[dragObj]:Disconnect()
	end
end
